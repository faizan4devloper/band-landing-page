...
.recent {
    ...
    border-radius: 10px;
    box-shadow: 0 10px 20px rgba(0, 0, 0, 0.2);
    width: 450px;  /* Set width for recent */
    height: 200px;  /* Set height for recent */
...
export function Chat({ onQuestionClick, onSchoolClick }) {
    ...
    const [expandedDescriptions, setExpandedDescriptions] = useState({});
    const [recent, setRecent] = useState([]); // New state for recent selections
    const [showRecent, setShowRecent] = useState(false); // New state for showing recent
    ...
    const handleQuestionClick = (question) => {
        ...
        onQuestionClick(question);
        updateRecent(question); // Update recent selections when a question is clicked
        setShowRecent(true); // Show recent selections
    };
    ...
    return (
        <div className="chat-container">
            ...
            <div className={`recent ${showRecent ? 'show' : 'hide'}`}>
                <h2>Recent Selections</h2>
                <ul>
                    {recent.map((item, index) => (
                        <li key={index}>
