...
const FaqDropdown = ({ contentData, onQuestionSelect, onGoClick, selectedQuestion }) => {
  ...
  const handleGoClick = (question, answer) => {
    onGoClick(question, answer); // Trigger Go action in Chatbot
  };
      ...
      <div className={`${styles.dropdownContent} ${isFaqOpen ? styles.show : styles.hide}`}>
        {contentData.map((item, index) => (
          ...
          >
            ...
            <button 
              onClick={() => handleGoClick(item.question, item.answer)} 
...
const Chatbot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Example of dynamically setting a question ID
  ...
  const handleGoClick = (question, answer) => {
    const botMessage = {
      type: 'bot',
      text: `Answer to your question: ${answer}`,
    };
    setMessages((prevMessages) => [...prevMessages, botMessage]);
  };
