...
const AllCardsPage = ({ cardsData }) => {
  ...
  // Function to generate tags based on active filters
  const generateTags = (card) => {
    const tags = [];
    if (filteredCards.some(filteredCard => filteredCard.industry === card.industry)) {
      tags.push(card.industry);
    }
    if (filteredCards.some(filteredCard => filteredCard.businessFunction === card.businessFunction)) {
            ...
            filteredCards.map((card, index) => (
              ...
              >
                <Cards
                  ...
                  tags={generateTags(card)} // Pass tags to Cards component
                />
              </div>
            ))
          ) : (
