...
export const cardsData = [
  {
    ...
    content: {
      ...
      adoption:"industry	Solution Adoption Financial	Can help in answering finance product information, Loan eligibility, Product Recommendation as per the email context Education	Can help students to get response to their queries related to Admission process, Scholarship schemes, Enrollment process etc. Healthcare/Insurance	Can help customers to understand their health insurance eligibility, claim processing, claim status etc. Retail	Queries related to product exchange / return can be better handled"
...
const MainContent = ({ activeTab, content }) => {
  ...
  const adoptionPoints = content.adoption.split(". ").map((point, index) => (
    <li key={index}>{point.trim()}</li>
  ));
