...
import { faSync, faFileAlt, faEye, faDownload, faRotateRight } from '@fortawesome/free-solid-svg-icons';
...
const DashboardTable = ({ userEmail }) => {
  ...
  const fetchDocuments = async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const params = { userEmail, pageSize };
      if (nextStartKey && !isRefresh) {
        params.startKey = nextStartKey;
      }

      const response = await axios.get(API_ENDPOINT, { params });
      const { data, nextStartKey: newNextStartKey } = response.data;

      setDocuments(data);
      setNextStartKey(newNextStartKey);
    } catch (error) {
  ...
  const renderTableContent = () => {
    ...
    return documents.map((document, index) => (
      ...
      >
        ...
        <td>
          <button onClick={() => handleMetadataClick(document.metadata)} className={styles.metadataButton}>
            View Metadata
          </button>
        </td>
  ...
  const handlePageChange = (direction) => {
    ...
    } else if (direction === 'next' && nextStartKey) {
      setCurrentPage(currentPage + 1);
    }
  };
