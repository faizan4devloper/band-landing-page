...
const QuestionBlock = ({ question, answerData }) => {
  ...
  const renderImage = (imageUrl, altText) => {
    if (!imageUrl) {
      console.warn("Image URL is missing:", altText);
      return null; // Return null if there is no image URL
    }
    return <img src={imageUrl} alt={altText} className={styles.responseImage} />;
  };
