...
const Chatbot = () => {
  ...
  const generateQuestionId = (questionText) => {
    return `question-${Date.now()}-${questionText.replace(/\s+/g, '-')}`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const userMessage = { type: 'user', text: input, id: questionId };
    ...
    try {
      ...
      const errorMessage = { type: 'bot', text: 'Something went wrong. Please try again later.', id: questionId };
        ...
        <div className={styles.chatWindow}>
          ...
          <div className={styles.messages}>
            {messages.map((message, index) => (
              <div key={message.id || index} className={message.type === 'user' ? styles.userMessage : styles.botMessage}>
