...
const Chatbot = ({ activeTopic }) => {
  ...
  // Dynamically define the questions based on activeTopic
  ...
  // Fetch data when a question is asked
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (input.trim() === '') return;

    const question = topicQuestions[activeTopic]?.find(q => q.text.toLowerCase().includes(input.trim().toLowerCase()));
