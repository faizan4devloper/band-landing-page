...
const MainContent = ({ activeTab, content }) => {
  ...
  const toggleMaximize = (imageSrc, section) => {
    const isMaximized = maximizedImage === imageSrc;
    setMaximizedImage(isMaximized ? null : imageSrc);
    if (!isMaximized) {
      setActiveCarousel(section); // Set the active carousel section when maximizing an image
    }
  };
    ...
    <div className={styles.carouselContainer}>
      ...
      >
        {images.map((image, index) => (
          <div
            key={index}
            onClick={() => toggleMaximize(image, "carouselSection")}
          >
    ...
    return content.length > 1
      ...
      : (
        <img
          ...
          onClick={() => toggleMaximize(content[0], section)}
  ...
  const renderImageOrCarousel = (images) => {
    ...
    return renderContent(images, activeCarousel);
  };
