...
const Chatbot = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Generate a unique ID for the request (can be a timestamp or a UUID)
    const requestId = new Date().toISOString(); // Use current timestamp as ID (or use a library like uuid)

    // Prepare the payload with question, user details (optional), and request ID
    const payload = {
      question: input,
      user_id: 'user_123',  // You can replace this with actual user information if available
      request_id: requestId,  // Adding the request ID for tracking
    };
