...
const App = () => {
  ...
  const handleClickLeft = () => {
    ...
    setBigIndex(newIndex); // Set the new card added on the left as big
  };

  const handleClickRight = () => {
    const newIndex = currentIndex === cardsData.length - 5 ? 0 : currentIndex + 1;
    setCurrentIndex(newIndex);
    setBigIndex(newIndex + 4); // Set the new card added on the right as big
  };
      ...
      >
        ...
        {visibleCards.map((card, index) => (
          <Cards
            ...
            isBig={currentIndex + index === bigIndex}
