...
const Card = ({ imageUrl, title, description, isBig, toggleSize, tags, hasData }) => {
  ...
  // Determine whether to display the "Read More" link or "No Data Available" message
...
// List of card titles that should display "No Data Available"
// Titles are matched exactly with the provided JSON data after removing spaces
const noDataCards = [
  "ResponsibleGenAIwithLlama-13B",
  "PredictiveAssetMaintenance​(PAM)​",
  "GraphdatainterpretationusingGenAI",
...
async function mapAssets(card) {
  ...
  // If it's a no-data card, set content to indicate no data available
  if (isNoDataCard) {
    return {
      ...card,
      imageUrl: images[card.imageUrl] || 'defaultImagePath.jpg', // Use a default image if the specified one is missing
...
// List of card titles that should display "No Data Available"
const noDataCards = ["ApiCase", "AaigApi", "ResponsibleGen", "PredictiveAsset", "GraphData"];
...
async function mapAssets(card) {
  ...
  // Check if the card is in the noDataCards list
  const isNoDataCard = noDataCards.includes(card.title);
