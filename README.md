...
import { faTimes, faChevronLeft, faChevronRight } from "@fortawesome/free-solid-svg-icons"; // Importing additional icons
import Video from "./Video";
import BeatLoader from "react-spinners/BeatLoader";

const MainContent = ({ activeTab, content }) => {
  const [maximizedImage, setMaximizedImage] = useState(null);
  const [maximizedSection, setMaximizedSection] = useState(null); // Track which section's image is maximized
  const [maximizedIndex, setMaximizedIndex] = useState(0); // Track the index of the maximized image
  const [currentSlide, setCurrentSlide] = useState(0);
  const [laserPos, setLaserPos] = useState({ x: 0, y: 0 });

  const sections = ["description", "solutionFlow", "techArchitecture", "benefits", "adoption"]; // Define the sections to cycle through

  // Toggles the maximization of the image when clicked
  const toggleMaximize = (imageSrc, section, index) => {
  ...
  const handleNextImage = () => {
    ...
    const sectionIndex = sections.indexOf(maximizedSection);
    const currentImages = content[maximizedSection];

    if (maximizedIndex + 1 < currentImages.length) {
      setMaximizedIndex(maximizedIndex + 1);
      setMaximizedImage(currentImages[maximizedIndex + 1]);
    } else if (sectionIndex + 1 < sections.length) {
      const nextSection = sections[sectionIndex + 1];
      setMaximizedSection(nextSection);
      setMaximizedIndex(0);
      setMaximizedImage(content[nextSection][0]);
    }
  };
  ...
  const handlePreviousImage = () => {
    ...
    if (maximizedIndex > 0) {
      ...
      const prevImages = content[prevSection];
      setMaximizedSection(prevSection);
      setMaximizedIndex(prevImages.length - 1);
      setMaximizedImage(prevImages[prevImages.length - 1]);
    }
  };

  // Add a mousemove event listener to track the cursor position when an image is maximized
  useEffect(() => {
    if (maximizedImage) {
  ...
  // Render a carousel with custom thumbnails
  const renderCarousel = (images, section) => (
    <div className={styles.carouselContainer}>
      ...
      >
        {images.map((image, index) => (
          <div
            key={index}
            onClick={() => toggleMaximize(image, section, index)} // Pass section and index for maximization
          >
  ...
  // Render either a single image or a carousel based on the content provided
  const renderContent = (content, section) => {
    ...
    return content.length > 1
      ? renderCarousel(content, section)
      : (
        <img
          src={content[0]}
          alt="Single Image"
          className={maximizedImage === content[0] ? styles.maximized : ""}
          onClick={() => toggleMaximize(content[0], section, 0)}
          title="Click To Enlarge"
        />
      );
  };

  // Render either the images or a loader while content is being fetched
  const renderImageOrCarousel = (images, section) => {
    ...
    return renderContent(images, section);
  };
  ...
  // Map the activeTab to the corresponding content section
  const contentMap = {
    description: (
      <div className={styles.description}>
        {renderImageOrCarousel(content.description, "description")}
      </div>
    ),
    solutionFlow: (
      <div className={styles.solution}>
        {renderImageOrCarousel(content.solutionFlow, "solutionFlow")}
      </div>
    ),
    demo: (
    ...
    techArchitecture: (
      <div className={styles.architecture}>
        {renderImageOrCarousel(content.techArchitecture, "techArchitecture")}
      </div>
    ),
    benefits: (
      <div className={styles.benefits}>
        {renderImageOrCarousel(content.benefits, "benefits")}
      </div>
    ),
    adoption: (
      <div className={styles.adoption}>
        {renderImageOrCarousel(content.adoption, "adoption")}
      </div>
    ),
  };

  return (
    // Apply the laser cursor and maximized image when the image is clicked
      ...
      {maximizedImage && (
        <div className={styles.overlay}>
          ...
          <div className={styles.navigationButtons}>
            <FontAwesomeIcon icon={faChevronLeft} onClick={handlePreviousImage} />
