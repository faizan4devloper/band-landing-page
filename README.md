...
async function mapAssets(card) {
  try {
    ...
    const data = {
      solutionFlow: assets[`${assetKey}solutionFlow`] || [],
      techArchitecture: assets[`${assetKey}techArchitecture`] || [],
      description: assets[`${assetKey}description`] || [],
      benefits: assets[`${assetKey}benefits`] || [],
      adoption: assets[`${assetKey}adoption`] || [],
      demo: assets[`${assetKey}demo`] || [],  // Fetch demo videos here
    };
    ...
    return {
      ...
      content: {
        ...
        demo: videos[data.demo] || null, // Ensure demo videos are correctly mapped
    ...
    console.error(`Failed to map assets for ${card.title}:`, error);
    return card; // Return the original card if there is an error
  }
}
